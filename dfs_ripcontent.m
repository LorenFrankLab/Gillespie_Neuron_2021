...
%     text([a+.3],[.8 ],num2str(ranksum(repcorr_errboxrates{a}(:,1),repcorr_errboxrates{a}(:,2)),'%.04f')); ylim([0 1]); ylabel('events/trial')
end
plot([0 11],[1 1],'k:'); ylim([0 2]); set(gca,'XTick',[3 9],'XTickLabel',{'search','repeat'});
%subplot(1,4,[2 3]); set(gca,'XTick',[3 9],'XTickLabel',{'future','pg'}); ylim([0 2])
% subplot(3,2,1); hold on; 
...
%% 8. measure effect of recency on replay rate; compare PGreplay rate on pg and nonpg visits   [FIG 5E and 7]
clearvars -except f animals animcol
...
%% 9. measure replay rate of an arm during contingencies before during/after it is goal [FIG 6]
clearvars -except f animals animcol
...
%% 10. epochwise boxplot version of violins [OUTER]  [Fig 8]
clearvars -except f animals animcol
bars = figure(); set(gcf,'Position',[66 305 1853 551]);
reps = 100;
mintrials=5;
...
for a = 1:length(animals)
    ...
    repcomb{a} = [rep1_pcg{a}(rep1_rtcounts{a}(:,2)>=mintrials)' rep1_prevgoal{a}(rep1_rtcounts{a}(:,2)>=mintrials)' rep1_other{a}(rep1_rtcounts{a}(:,2)>=mintrials)' rep1_totalother{a}(rep1_rtcounts{a}(:,2)>=mintrials)' rep1_box{a}(rep1_rtcounts{a}(:,2)>=mintrials)' ];
    repshuffcomb{a} = reshape(rep1_randshuff{a}(rep1_rtcounts{a}(:,2)>=mintrials,:),[],1); 
...
subplot(1,8,[1:4]); hold on; title('outer rep')
plot4a(repcomb,'gnames',{'f/past/cg','pg','other','totalother', 'box'});
...
%% 11. plot outerreps per animal [Sup1A]
figure; set(gcf,'Position',[369 473 1387 408]); hold on; xlabel('epoch'); ylabel('Goal repeats');
converter = [0 0 0 1 0 0 0 0 0 2 0 0 0 0 3];
for a = 1:length(animals)
    animalinfo = animaldef(animals{a});
    task = loaddatastruct(animalinfo{2},animals{a},'task');
    eps = evaluatefilter(task,'$ripthresh>=0 & isequal($environment,''goal'')'); %
    for e = 1:size(eps,1)
        reps{a}(e) = task{eps(e,1)}{eps(e,2)}.outerreps;
    end
    subplot(2,2,a); hold on; plot(converter(reps{a}),'.-','LineWidth',2,'Color',animcol(a,:),'MarkerSize',15)
    ylim([.5 3.5]); xlim([0 45]); set(gca,'ytick',[1 2 3],'yticklabel',{'4-12','10','15'}); title(animals{a});xlabel('epoch'); ylabel('outerreps');
    % plot(reps{a},'o-','LineWidth',2,'Color',animcol(a,:)); title(animals{a});  %plot([0 45; 0 45]',[2 2; 3 3]','k:');
end
%legend(animals); 

%% 12. plot the time spent, numrips, and rip rate at each trial phase - home, RW, postRW, outer(rew), outer(unrew) [SupFig 1C]
figure;
...
%% 13. plot the fraction of each event in the current maze segment and max remote single arm fraction [Supfig 3H]
figure;
for a = 1:length(animals)
    eps = find(arrayfun(@(x) ~isempty(x.trips),f(a).output{1}));
    for e = 1:length(eps)
...
%% 14. plot rip features (size, duration, instfreq, spikerate, numtets) by cont/frag and by local/remote and corr/err [Supfig 5]
clearvars -except f animals animcol
...
for a = 1:length(animals)
    ...
    for e = 1:length(eps)
        ...
        for t=1:length(rips)  % categorize each events as cont/frag and remote/local; use time of event to add other features [c/f r/l amp dur instfrq spkrate tetnum] 
            if ~isempty(rips{t})
                ...
                events{t} = [valid,ind-1==0, counts{t}(:,7:8), riptsz(ripinds,2), riptsz(ripinds,3)./counts{t}(:,8)./bytet.out{a}.totaltetnum{de(1)}{de(2)},riptsz(ripinds,4)];
            else events{t} = []; end
            if ~isempty(outerrips{t})
                [maxval,ind] = max(outerrips{t},[],2); %(:,2:end)
                valid = outertypes{t}'==1 & maxval>contentthresh;
                ripinds = lookup(outercounts{t}(:,6),riptsz(:,1));
                outerevents{t} = [valid,ind-1==trialstack(t,1),outercounts{t}(:,7:8), riptsz(ripinds,2), riptsz(ripinds,3)./outercounts{t}(:,8)./bytet.out{a}.totaltetnum{de(1)}{de(2)},riptsz(ripinds,4)]; 
    ...
    wrds = sprintf('pooled\np=%.03f',ranksum(corrspk, errspk)); text(a,25,wrds);
...
subplot(2,5,4); hold on; for a=1:4
    ...
    title('spkrate'); ylim([0 200]); xlim([0 5]); text(a,20+10*a,num2str(pvals(1))); end
...
text(a,20+10*a,num2str(pvals(2))); ylim([0 200]); xlim([0 5]); end
...
%% 15. make boxplot quantification of replay fracs that are past, future, pg, etc & GLM plot % corr vs error for LAST EVENT ONLY [SupFig8]
clearvars -except f animals animcol
...
%                 end
    ...
    s_mdl = fitglm(searchtbl,'linear','Distribution','poisson');
    CI = coefCI(s_mdl,.01);
    subplot(2,2,1); hold on; title('last remote event, search')
    %errorbar(a+[0:5:19],table2array(mdl.Coefficients(:,1)), table2array(mdl.Coefficients(:,2)),'.','Color',animcol(a,:))
    plot(a+[0:5:19],exp(table2array(s_mdl.Coefficients(:,1))),'.','MarkerSize',20,'Color',animcol(a,:));
    plot([a+[0:5:19];a+[0:5:19]],exp(CI)','Color',animcol(a,:));
    %cr = [corrcoef(searchcat(:,1),searchcat(:,2)), corrcoef(searchcat(:,1),searchcat(:,3)), corrcoef(searchcat(:,2),searchcat(:,3))];
    ...
    r_mdl = fitglm(reptbl,'linear','Distribution','poisson'); 
    CI = coefCI(r_mdl,.01);
    subplot(2,2,2); hold on; title('last remote event, repeat')
    %errorbar(a+[0:5:24],table2array(mdl.Coefficients(:,1)), table2array(mdl.Coefficients(:,2)),'.','Color',animcol(a,:))
    plot(a+[0:5:24],exp(table2array(r_mdl.Coefficients(:,1))),'.','MarkerSize',20,'Color',animcol(a,:));
    plot([a+[0:5:24];a+[0:5:24]],exp(CI)','Color',animcol(a,:));
    %cr = [corrcoef(repeatcat(:,1),repeatcat(:,2)), corrcoef(repeatcat(:,1),repeatcat(:,3)),corrcoef(repeatcat(:,1),repeatcat(:,4)), ...
    ...
    text(a,.4,wrds);  ylim([0 .5]); ylabel('frac trials'); title('last ev is future')
    subplot(2,2,4); hold on;
    ...
    text(a,.4,wrds);  ylim([0 .5]); ylabel('frac trials'); title('last ev is currgoal')  
end
subplot(2,2,1); plot([0 20],[1 1],'k:'); set(gca,'YScale','log'); ylim([.3 3]);
subplot(2,2,2); plot([0 25],[1 1],'k:'); set(gca,'YScale','log'); ylim([.3 3]);
...
%% 16. quantify contingency corelations between pgoal numreps and pgoal replays  
...
%print(figname,'-depsc','-painters')           %% make Violins and bars of content relationships with various trial categories, vs rand shuffs [HOME / RW] 
clearvars -except f animals animcol
bars = figure(); set(gcf,'Position',[66 305 1853 551]);
reps = 5000;
contentthresh = .3;
...
%% ------OTHER EXPLORATORY STUFF, not in manuscript directly ----------
...
%% [original] make Violins and bars of content relationships with various trial categories, vs rand shuffs [OUTER]  
clearvars -except f animals animcol
bars = figure(); set(gcf,'Position',[66 305 1853 551]);
reps = 5000;
contentthresh=.3;
...
%%  compare rip sizes with rippleskons_low
figure
for a =1:length(animals)
    eps = find(arrayfun(@(x) ~isempty(x.trips),f(a).output{1}));
    for e = 1:length(eps)
...
%% how likely is replay to precede a visit to a given arm?
clearvars -except f animals animcol
figure(); set(gcf,'Position',[66 305 1853 551]);
contentthresh = .3;
reps = 100;
for a = 1:length(animals)
    ...
    for e = 1:length(eps)
        ...
        for t=1:length(rips)
            ...
            isrewarm(t) = ismember(outers(t),goals(1:t,2));
        end
        % search trials only 
        % what fraction of visits are preceded by replay during search vs if you shuffle replay/visits 
        search_outers = outers(tphasenum(:,1)<=1 & ~isrewarm');
        previsreplay{a}(e,1) = mean(cellfun(@(x,y) ismember(y,x),replays(tphasenum(:,1)<=1 & ~isrewarm'),num2cell(search_outers)));
        for r = 1:reps
            previsshuff{a}(e,r) = mean(cellfun(@(x,y) ismember(y,x),replays(tphasenum(:,1)<=1 & ~isrewarm'),num2cell(search_outers(randperm(length(search_outers))))));
        end
        % does you get more/less predictive replay if you're going back to where you just searched?
        repeatsearch = find(pastwlock==outers' & tphasenum(:,1)<=0);
        if any(repeatsearch)
            repsearch{a}{e} = cellfun(@(x,y) sum(y==x),replays(repeatsearch),num2cell(outers(repeatsearch)));
        else repsearch{a}{e} = [];
        end
        norepeatsearch = find(pastwlock~=outers' & tphasenum(:,1)<=0);
        norepsearch{a}{e} = cellfun(@(x,y) sum(y==x),replays(norepeatsearch),num2cell(outers(norepeatsearch)));
    end
    previs_p(a) = ranksum(previsreplay{a},reshape(previsshuff{a},[],1));
    repsearch_p(a) = ranksum(horzcat(repsearch{a}{:}),horzcat(norepsearch{a}{:}));
    
    hold on
    errorbar(a+[0 .3],[mean(previsreplay{a}),mean2(previsshuff{a})],[std(previsreplay{a})./sqrt(length(previsreplay{a})), std2(previsshuff{a})./sqrt(reps*length(previsreplay{a}))],'k.');
    bar(a+[0 .3],[mean(previsreplay{a}),mean2(previsshuff{a})],.8,'FaceColor',animcol(a,:));
    text(a,.5,num2str(previs_p(a)))
end
%subplot(2,2,1); hold on; title('future replay during search') 
%plot4a(cellfun(@(x) reshape(x,[],1),previsshuff,'un',0),'gnames',{'shuffle'})
%ylim([0 1]); ylabel('frac search visits preceded by replay')
%subplot(2,2,2); hold on;
%plot4a(previsreplay,'gnames',{'frac with replay'}); ylim([0 1])
%text(4,.7,num2str(previs_p))
%subplot(2,2,3); hold on;
%plot4a(cellfun(@(x) horzcat(x{:})',repsearch,'un',0),'gnames',{'repeat'}) 
%ylabel('replay rate'); text(4,.7,num2str(repsearch_p))
%subplot(2,2,4); hold on;
%plot4a(cellfun(@(x) horzcat(x{:})',norepsearch,'un',0),'gnames',{'diff'})
