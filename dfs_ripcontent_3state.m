...
f = setfilterfunction(f, 'dfa_ripcontent', {'ripdecodesv2','trials','pos'});
f = runfilter(f);
...
for a = 1:length(animals)
    ...
    for e = 1:length(eps)
        ...
        % MAXSTATES 0=nonclassified, 1=cont 2=frag 3=hover 
        for t=1:length(homerips)  % extract valid rips and tack on trial info: [replay future past currgoal prevgoal ppgoal tphase salient local TYPE(cont/stat)]
            if ~isempty(homerips{t})
                [maxval,ind] = max(homerips{t},[],2); %(:,2:end)
                valid = (hometypes{t}'==1 | hometypes{t}'==3) & maxval>contentthresh;                
                ...
                homereplays{t} =[ind(valid)-1,repmat(trialstack(t,:),sum(valid),1), ismember(ind(valid)-1,[unique(goals(1:t,1))']),ind(valid)-1==0,hometypes{t}(valid)'];
            else homereplays{t} = []; end
            if ~isempty(rwrips{t})
                [maxval,ind] = max(rwrips{t},[],2); %(:,2:end)
                valid = (rwtypes{t}'==1 | rwtypes{t}'==3) & maxval>contentthresh;
                ...
                rwreplays{t} = [ind(valid)-1,repmat(trialstack(t,:),sum(valid),1), ismember(ind(valid)-1,[unique(goals(1:t,1))']),ind(valid)-1==0, rwtypes{t}(valid)'];
            else rwreplays{t} = []; end
            if ~isempty(postrwrips{t})
                [maxval,ind] = max(postrwrips{t},[],2); %(:,2:end)
                valid = (postrwtypes{t}'==1 | postrwtypes{t}'==3) & maxval>contentthresh;
                ...
                postrwreplays{t} = [ind(valid)-1,repmat(trialstack(t,:),sum(valid),1), ismember(ind(valid)-1,[unique(goals(1:t,1))']),ind(valid)-1==0, postrwtypes{t}(valid)'];
            ...
            if ~isempty(outerrips{t})   % box is considered salient in option 1, not in option 2
                ...
                valid = (outertypes{t}'==1 | outertypes{t}'==3) & maxval>contentthresh;
                ...
                outerreplays{t} = [ind(valid)-1,repmat(trialstack(t,:),sum(valid),1), ismember(ind(valid)-1,[unique(goals(1:t,1))']),ind(valid)-1==trialstack(t,1),outertypes{t}(valid)']; % include box as salient
                if any(valid)
    ...
    totalnums{a} = [sum(cellfun(@(x) sum(x(:,9)==1 & x(:,10)==1),allbox{a})), sum(cellfun(@(x) sum(x(:,9)==1 & x(:,10)==3),allbox{a})), ...
        sum(cellfun(@(x) sum(x(:,9)==0 & x(:,10)==1),allbox{a})), sum(cellfun(@(x) sum(x(:,9)==0 & x(:,10)==3),allbox{a})), ...
...
%%  measure replay rate of an arm during contingencies before during/after it is goal [FIG 5]
clearvars -except f animals animcol
...
%% Calc mean rate of local, past, future, pg replay by trial (box vs outer & search vs rep)  [SUP FIG 5A]
clearvars -except f animals animcol
