...
animals = {'despereaux'}; %{'remy','gus','bernard','fievel'}; %
...
epochfilter{1} = ['$session==4 & isequal($type,''run'')'];
...
tet = [25 26 30]; % desp[18,20,24]
%tet = [12,18,27]; % jaq
%tet = [6 14 18]; % roqui
%tet = [13 15 18]; %monty 2 7 12

f = createfilter('animal',animals,'epochs',epochfilter,'excludetime', timefilter, 'iterator', iterator);
f = setfilterfunction(f, 'dfa_plotripcontent_sg', {'ripdecodesv3','trials','marks','tetinfo'},'animal',animals{1},'posterior',1,'v',3,'tet',tet,'span','full');
f = runfilter(f);
