function out = dfa_likcontent(index, excludeperiods, ripdecodes,trials, pos, varargin)
...
if isempty(ripdecodes) || length(ripdecodes{d})< e || isempty(ripdecodes{d}{e})
    ...
    grps(~nongap)=nan;
    armedges = [find(armstarts); length(nongap)+1];
    if max(grps)<9
        ...
        q = ripdecodes{d}{e};
        slope = nan(size(q.riptimes,1),1);
        for r = find(q.max_state(:,1)==1 & max(q.arm_prop,[],2)>.3 )' % only look at continuous events with density concentrated in an arm
            postinds = posteriorts>q.riptimes(r,1) & posteriorts<q.riptimes(r,2);
            postchunk = post_combined(:,postinds);
            % stack the max arn and the box rows, calculate slope
            [maxval,maxarm] = max(q.arm_prop(r,2:end));
            postchunk_reduced = postchunk(grps==1 | grps==maxarm+1,:);
            [map,indofmap] = max(postchunk_reduced);
            [tmpslope,intr] = polyfit(1:length(map),indofmap,1);
            slope(r) = tmpslope(1)*posbins/timebins; % convert to cm/s - 5cm posbins, .002s timebins
            
            %figure; imagesc(postchunk_reduced); set(gca,'YDir','normal'); hold on;
            %plot(1:length(map),indofmap','r.'); lsline; title(sprintf('box=%.02f,arm=%.02f,slope%.02f',q.arm_prop(r,1),q.arm_prop(r,maxarm+1),slope(r)))
        end
        
    % no states -> label all ones
    riplabels = ones(size(q.riptimes,1),1); 
    ripmaxlabels = riplabels;
    ...
    out.riptsz = [q.riptimes, q.ripsizes, riplabels, ripmaxlabels, box, trialphaselabels, boxfrac,slope];
    ...
    trips.homeripspeed = cell(1,length(nonlocktrials));
    ...
    trips.RWripspeed = cell(1,length(nonlocktrials));
    ...
    trips.postRWripspeed = cell(1,length(nonlocktrials));
    ...
    trips.outerripspeed = cell(1,length(nonlocktrials));
    ...
    for t = find(nonlocktrials)'
        ...
        trips.homeripspeed{t} = slope(find(isExcluded(q.riptimes(:,1),[trials{d}{e}.starttime(t), trials{d}{e}.RWstart(t)])),:);
        ...
        trips.RWripspeed{t} = slope(find(isExcluded(q.riptimes(:,1),[trials{d}{e}.RWstart(t), trials{d}{e}.RWend(t)])),:);
        ...
        trips.postRWripspeed{t} = slope(find(isExcluded(q.riptimes(:,1),[trials{d}{e}.RWend(t), trials{d}{e}.leaveRW(t)])),:);
        ...
        trips.outerripspeed{t} = slope(find(isExcluded(q.riptimes(:,1),[trials{d}{e}.outertime(t), trials{d}{e}.leaveouter(t)])),:);
